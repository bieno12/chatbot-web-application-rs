use crate::models;
use actix_web::{body::BoxBody, http::header::HttpDate, HttpResponse, Responder};
use rusqlite::params;
...
impl Message {
    pub fn get(id: i32) -> Result<Self, crate::Error> {
        ...
        let mut statement: rusqlite::Statement<'_> = conn.prepare(
            "SELECT id, created_at, content, conv_id, sender FROM messages WHERE id = ?",
        )?;
